...
def get_dataset_by_name(specs_name, data_dir, mode=None):
        ...
        for size in size_list:
            ...
            for filename in os.listdir(directory):
                if filename.endswith(".npy"):
                    ...
                    if mode == 'marginal':
                        graph.set_ground_truth(unary_marginal_est=data_dict["unary_marginal"])
                    
                    elif mode == 'map':
                        graph.set_ground_truth(unary_marginal_est=data_dict["unary_marginal"],
                                                pairwise_marginal_est=data_dict["pairwise_marginal"],
                                                map_est=data_dict["map"],
                                                unary_map_onehot=data_dict["unary_map_one_hot"],
                                                pairwise_map_onehot=data_dict["pairwise_map_one_hot"])
                    graph.struct = struct
                    #print(count)
                    #if count <= 200:
                    graphs.append(graph)
    ...
    if mode is not None:
        if mode == 'marginal':
            graphs = [g for g in graphs if getattr(g, 'unary_marginal') is not None]
        else:
