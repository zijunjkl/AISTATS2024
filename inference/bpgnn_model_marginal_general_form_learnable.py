...
class BPGNN(nn.Module):
    ...
    def forward(self, J, bias, N_step, MPNNtype):
        ...
        for step in range(N_step):            
            ...
            ln_mu_ji_scale = torch.sum(ln_mu_ji_scale, dim=1)      
        ...
        temp = readout * torch.log(readout + 1e-5)
        node_Hvalue = -1.0 * temp.sum(dim=1)
        
        temp = readout_pairwise * torch.log(readout_pairwise + 1e-5)
        ...
        return readout, readout_pairwise, [], entropy_, node_entropy, edge_entropy, Ci[:,0], Cij, dom_scaler
